...
function updateDepth(book, newPage) {
    var page = book.turn('page'),
        ...
        depthWidthStart = 16 * Math.min(1, page * 2 / pages),
        depthWidthEnd = 16 * Math.min(1, (pages - page) * 2 / pages);

    newPage = newPage || page;

    // ✅ الجهة اليمنى (اللي اتقلبت فعلاً)
    if (newPage > 3) {
        $('.sj-book .front-side .depth').css({
            width: depthWidthStart,
            right: 20 - depthWidthStart,
            left: 'auto'
        });
    } else {
        $('.sj-book .front-side .depth').css({ width: 0 });
    }

    // ✅ الجهة اليسرى (اللي لسه ما اتقلبتش)
    if (newPage < pages - 3) {
        $('.sj-book .back-side .depth').css({
            width: depthWidthEnd,
            left: 20 - depthWidthEnd,
            right: 'auto'
        });
    } else {
        $('.sj-book .back-side .depth').css({ width: 0 });
    }
}
